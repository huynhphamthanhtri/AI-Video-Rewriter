...
export const geminiEdlSchema = z.object({
  ...
  video_segments: z.array(z.object({ segment_id: z.number().int().positive(), order: z.number().int().positive(), source_id: z.string().optional(), source_start: z.string().regex(clipTime), source_end: z.string().regex(clipTime), subtitle_start: z.number().int().positive(), subtitle_end: z.number().int().positive(), scene_description: z.string() })).min(1),
});
