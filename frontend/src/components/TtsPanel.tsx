...
export function TtsPanel({ renderOptions, onRenderOptionsChange }: { renderOptions: RenderOptions; onRenderOptionsChange: (value: RenderOptions) => void }) {
  ...
  const voiceGroups = [
    ...
    { label: 'English US', code: 'en', localePrefix: 'en-US' },
    { label: 'Deutsch', code: 'de', localePrefix: 'de-' },
    { label: '日本語', code: 'ja', localePrefix: 'ja-' },
    { label: 'Español México', code: 'es', localePrefix: 'es-MX' },
    { label: '한국어', code: 'ko', localePrefix: 'ko-' },
  ];
        ...
        <div className="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
          ...
          <div className="flex items-center gap-3">
            <select className="select-ghost flex-1" value={renderOptions.tts_mode} onChange={e => updateOptions({ tts_mode: e.target.value as RenderOptions['tts_mode'] })}>
              ...
              <option value="voiceover">Bật Edge TTS đọc theo srt[]</option>
            </select>
          </div>
        </div>
