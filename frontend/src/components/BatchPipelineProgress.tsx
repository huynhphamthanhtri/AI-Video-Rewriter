  ...
  return <div className="mt-3 rounded-xl border border-violet-500/20 bg-violet-500/10 p-3">
    ...
    {renderStatus.completed_segments != null && renderStatus.total_segments != null && (
      <p className="mt-1 text-[11px] text-slate-400">
        Đoạn: {renderStatus.completed_segments}/{renderStatus.total_segments}
      </p>
    )}
  </div>;
}
...
export function BatchPipelineProgress({ progress, onCancel }: { progress: BatchProgress | null; onCancel: () => void }) {
  ...
  return <div className="rounded-2xl border border-fuchsia-500/20 bg-fuchsia-500/10 p-4">
    <div className="mb-3 flex items-start justify-between gap-3">
      <div>
        <h3 className="flex items-center gap-2 font-bold text-fuchsia-200"><ExternalLink size={18} />Xử lý hàng loạt</h3>
        <p className="mt-1 text-xs text-slate-400">{progress.total_items} video · Video {Math.min(progress.current_index + 1, progress.total_items)}/{progress.total_items} · {statusLabel(progress.status)}</p>
      </div>
    ...
    <div className="mt-3 grid gap-2 text-xs text-slate-300 md:grid-cols-4">
      ...
      <span>Đã hủy: {cancelledCount}</span>
      <span>Đã xử lý: {processed}/{progress.total_items}</span>
    </div>
      ...
      {progress.items.map(item => <div key={`${item.index}-${item.source_url}`} className="rounded-xl border border-white/10 bg-slate-950/40 p-3">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {Boolean(item.result?.final_video_path) && <p className="mt-2 break-all text-xs text-emerald-300">File: {String(item.result?.final_video_path)}</p>}
          </div>
        </div>
